...
export default function SimilarSymptoms({ symptom, onClose, onRefresh }) {
  ...
  const [lowerSimilarityCases, setLowerSimilarityCases] = useState([]);
  const [showLowerSimilarity, setShowLowerSimilarity] = useState(false);
      ...
      <div className="bg-white rounded-lg shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          */}
          ...
          {/* Lower Similarity Cases - Show only if no high similarity matches found */}
          {!loading && similarCases.length === 0 && lowerSimilarityCases.length > 0 && (
            <div className="mt-6">
              <button
                onClick={() => setShowLowerSimilarity(!showLowerSimilarity)}
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-gray-700 font-medium">
                    Some similar cases that may relate
                  </span>
                  <span className="text-xs bg-gray-200 text-gray-600 px-2 py-1 rounded">
                    {lowerSimilarityCases.length} case{lowerSimilarityCases.length !== 1 ? 's' : ''}
                  </span>
                </div>
                <svg
                  className={`w-5 h-5 text-gray-500 transition-transform ${showLowerSimilarity ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <div className="mt-4 space-y-4">
                  {lowerSimilarityCases.map((similar) => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <h4 className="font-semibold text-gray-800">{similar.title}</h4>
                        <span className="text-xs bg-gray-200 text-gray-600 px-2 py-1 rounded font-semibold">
                          {Math.round(similar.similarityScore * 100)}% match
                        </span>
                      </div>
                      <p className="text-sm text-gray-600 mb-2 line-clamp-2">
