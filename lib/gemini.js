...
3. Include ALL matches >= 0.7 (don't artificially limit)
4. If no matches >= 0.7 exist, ask the user if they want to see posts with a lower similarity rating 
5. Order by similarity (highest first)
6. Always provide reasoning explaining the match, the reasoning should be in a user-friendly manner (brief, simplistic but explanatory, no overly technical terms)

## RESPONSE FORMAT (JSON only, no markdown):
{
  "matches": [
    {"index": 0, "similarity": 0.95, "reasoning": "Both describe severe headache with nausea in the morning"},
    {"index": 2, "similarity": 0.75, "reasoning": "Same headache symptom1 but different severity"}
  ]
}
